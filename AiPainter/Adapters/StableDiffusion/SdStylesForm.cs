...
namespace AiPainter.Adapters.StableDiffusion
{
    public partial class SdStylesForm : Form
    {
        private static string BasePath =>
            Path.Join(Application.StartupPath, "stable_diffusion_styles");
        ...
        private readonly ImageList imageList =
            new() { ImageSize = new Size(128, 128), ColorDepth = ColorDepth.Depth32Bit };
        ...
        public SdStylesForm()
        {
            ...
            {
                categories = JsonSerializer.Deserialize<SdModCategory[]>(
                    File.ReadAllText(jsonFile)
                )!;
                lbCategory.Items.Clear();
                // ReSharper disable once CoVariantArrayConversion
                lbCategory.Items.AddRange(
                    categories.Select(x => x.category).Distinct().OrderBy(x => x).ToArray()
                );
            }
        ...
        private void lbCategory_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            foreach (
                var modifier in categories
                    .Single(x => x.category == (string)lbCategory.SelectedItem!)
                    .modifiers.Select(x => x.modifier)
            )
            {
                ensureImageLoaded(modifier);
        ...
        {
            if (lvModifiers.SelectedItems.Count == 0)
                return;
        ...
        {
            if (imageList.Images.ContainsKey(modifier))
                return;

            var m = categories
                .SelectMany(x => x.modifiers)
                .FirstOrDefault(x => x.modifier == modifier);
            if (m == null)
                return;

            var preview =
                m.previews.SingleOrDefault(x => x.name == previewName) ?? m.previews.First();
